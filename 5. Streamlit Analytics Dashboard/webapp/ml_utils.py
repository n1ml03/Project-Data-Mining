...
class BinaryClassifiersAnalysis:
    ...
    def fit(self, classifiers, X, y, approach='', random_search=False, scoring='roc_auc', cv=5, verbose=5, n_jobs=-1):
        ...
        :param approach: string to be added on model's name as suffix for identifying purposes [type: string, default: '']
        ...
        :param n_jobs: n_jobs param from RandomizedSearchCV [type: int, default: -1]
    ...
    def evaluate_performance(self, X_train, y_train, X_test, y_test, cv=5, save=False, overwrite=True,
        ...
        for model_name, model_info in self.classifiers_info.items():
            ...
            if 'train_performance' in model_info.keys():
                df_performances = pd.concat([df_performances, model_info['train_performance']], ignore_index=True)
                df_performances = pd.concat([df_performances, model_info['test_performance']], ignore_index=True)
                continue
...
def elbow_method_kmeans(df, K_min, K_max, figsize=(16, 6)):
    ...
    for k in range(K_min, K_max + 1):
        ...
        pd.concat([square_dist, km.inertia_])
