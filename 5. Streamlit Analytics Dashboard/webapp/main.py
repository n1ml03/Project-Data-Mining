...
def product_and_customer_behavior_analysis():
    ...
    st.markdown('Orders count across **days of week and hourly** comparison')
    ...
    fig.update_layout(title="Sales Each Month & Each Day of Week",
                      xaxis_title="Months",
    ...
    st.markdown('**_Customer_ Rating** Each Month & Day of Week')
    fig = px.line(review_score_per_month, x="review_month", y="review_score", color='review_day',
                  title='Ratings Each Month & Day of Week')
    ...
    fig = px.sunburst(total_rev_month, path=['Sales Year', 'Product Category'], values='Sales Revenue',
                      color='Sales Revenue', hover_data=['Sales Revenue'],
                      color_continuous_scale='RdBu',
                      color_continuous_midpoint=np.average(total_rev_month['Sales Revenue'],
                                                           weights=total_rev_month['Sales Revenue']))
    st.write(fig)
    st.markdown("""
    Quan sát tổng thể:
    ...
    fig.update_layout()  # Optional: Adjust layout if needed
    st.write(fig)
...
def customer_segmentation_and_ltv():
    ...
    data = [trace0]
    layout = go.Layout(title='Avg days between first and last purchase')
...
def geography_analysis():
    ...
    # 2. Customer Review and Other Geography-related Visualizations (images)
    st.markdown('**Customer Review Across Brazil**')
    ...
    # st.markdown('**_Revenue_ Brazil** Across Brazil')
    # image9 = Image.open('./images/revenue.png')
    # st.image(image9, use_column_width=True)
    # st.markdown("""
    #
    # """)
    st.markdown('**E-Commerce on Brazilian State**')
...
# Main Streamlit App
def main():
    menu = ["Product & Customer Behaviour Analysis", "Customer Segmentation & LTV",
            "Geography Analysis", "Recommendation System", "Review Sentiment"]
